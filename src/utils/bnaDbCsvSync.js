...
const BNA_SOURCE_KEY = 'bna';

/** מיפוי ל־DB.csv — אנגלית מכותרת דף הבית/ RSS; he/ar מתרגום. */
export function buildBnaDbCsvUpdates({ hero, flashers, homeUrl, titleTranslations = {}, imageHeadline = '' }) {
  ...
  if (!sourceUrl) sourceUrl = 'https://www.bna.bh/en';
...
export async function syncBnaRowToDbCsv(patch) {
  return updateDbCsvRowBySourceKey(BNA_SOURCE_KEY, patch);
}
