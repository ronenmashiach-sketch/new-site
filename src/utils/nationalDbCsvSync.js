...
const NATIONAL_SOURCE_KEY = 'national';

/** מיפוי ל־DB.csv — מקור באנגלית, תרגום ל־he/ar (כמו ahram). */
export function buildNationalDbCsvUpdates({
  hero,
  flashers,
  homeUrl,
  titleTranslations = {},
  ...
  if (!sourceUrl) sourceUrl = 'https://www.thenationalnews.com';
...
export async function syncNationalRowToDbCsv(patch) {
  return updateDbCsvRowBySourceKey(NATIONAL_SOURCE_KEY, patch);
}
