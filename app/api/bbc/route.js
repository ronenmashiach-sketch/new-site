...
import { buildBbcDbCsvUpdates, syncBbcRowToDbCsv } from '@/utils/bbcDbCsvSync';
import { BBC_HOME_URL, BBC_RSS_URL, buildBbcNewsPayload } from '@/utils/bbcNewsPayload';
...
function isBbcFamilyUrl(urlString) {
  try {
...
/**
 * GET /api/bbc — Homepage hero (bbc.com) + RSS flashers + תרגום (ברירת מחדל translate=he,ar).
 * פרמטרים: rssUrl, homeUrl, flashers, translate, translateFlashers, useDbFallback.
 */
export async function GET(request) {
  try {
    const { searchParams } = request.nextUrl;
    const rssUrl = (searchParams.get('rssUrl') && searchParams.get('rssUrl').trim()) || BBC_RSS_URL;
    const homeUrl = (searchParams.get('homeUrl') && searchParams.get('homeUrl').trim()) || BBC_HOME_URL;

    if (!isBbcFamilyUrl(rssUrl) || !isBbcFamilyUrl(homeUrl)) {
      return Response.json({ error: 'מותר רק דומיין BBC ל־rssUrl ול־homeUrl' }, { status: 400 });
    }
    ...
    try {
      ...
      const row = rows.find((r) => String(r.source_key || '').trim().toLowerCase() === 'bbc');
      const cached = buildFromCachedDbRow(row);
      if (!cached) {
        return Response.json(
          { error: 'אין נתונים ב-DB.csv ל־bbc', fetchError },
          { status: 502, headers: { 'Content-Type': 'application/json; charset=utf-8' } },
        );
      }
      return Response.json(
    ...
    const csvPatch = buildBbcDbCsvUpdates({
      hero,
      flashers,
      homeUrl,
    ...
    try {
      await syncBbcRowToDbCsv(csvPatch);
      dbCsvSynced = true;
    } catch (err) {
      dbCsvSyncError = String(err?.message || err);
      console.error('api/bbc DB.csv sync:', err);
    }
